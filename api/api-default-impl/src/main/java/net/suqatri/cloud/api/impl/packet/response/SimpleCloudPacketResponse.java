...
public class SimpleCloudPacketResponse extends CloudPacketResponse {
    ...
    @Override
    public void receive() {
        if(this.getThrowable() == null) return;
        ...
        FutureAction<ICloudPacket> futureAction = packetData.getResponseAction();
        if(!futureAction.isCompletedExceptionally() && !futureAction.isDone() && !futureAction.isCancelled()){
            futureAction.completeExceptionally(this.getThrowable());
        }
    }
}
